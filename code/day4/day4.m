...
ylabel('Angle [deg]');
% Plot results
%load ('measurements.mat'); load ('inputs.mat');
...
xlim([4 (N+2*n_offset)*h]); ylim([-50 210]);
subplot(2,1,2); hold all;
plot(t_real,elevation, 'LineWidth', 2,'LineStyle','--');
legend('x_e^*','u_e^*','Constraint','x_e');
line([offsetTime offsetTime],get(gca,'YLim'),'Color','Black','LineWidth',1);
line([offsetTime+N*h offsetTime+N*h],get(gca,'YLim'),'Color','Black','LineWidth',1);
xlim([4 (N+2*n_offset)*h]); ylim([-5 14]);
% Set the dimensions of the figure
set(fig, 'units','centimeters'); pos = get(gcf, 'position'); set(gcf, 'position', [pos(1), pos(2), 15, 12]);