...
loadM = load (sprintf('../measurements/day4/measurements_q_%d_%d_%d_%d_%d_%d.mat', Q_LQR(1,1), Q_LQR(2,2), Q_LQR(3,3), Q_LQR(4,4), Q_LQR(5,5), Q_LQR(6,6) ));
measurements = loadM.simout_measurements;
loadI = load (sprintf('../measurements/day4/inputs_q_%d_%d_%d_%d_%d_%d.mat', Q_LQR(1,1), Q_LQR(2,2), Q_LQR(3,3), Q_LQR(4,4), Q_LQR(5,5), Q_LQR(6,6) ));
inputs = loadI.simout_measurements;

sim_travel = z(1:nx:N*nx);
sim_elev   = z(5:nx:N*nx);
pitch_ref  = z(N*nx+1:nu:N*(nx+nu));
elev_ref   = z(N*nx+2:nu:N*(nx+nu));
t_real = measurements.time-N/2;
travel = (180/pi)*measurements.signals.values(:,1);
elevation = (180/pi)*measurements.signals.values(:,5);
pitchInput = (180/pi)*inputs.signals.values(:,1);
elevationInput = (180/pi)*inputs.signals.values(:,1);
